...
SAMPLE_RATE = 2.4e6           # Hz (leave adjustable if you want to change per device)
FFT_SIZE    = 8192*5
GAIN_TUNER  = 30
GAIN_MIXER  = None
GAIN_LNA    = None
...
class SDRWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        if not driver:
            print("[SDR] No known device found, forcing rtl fallback")
            args = {"driver": "rtlsdr"}
            driver = "rtlsdr"
        ...
        if driver == "rtlsdr":
            ...
            sdr.setSampleRate(SOAPY_SDR_RX, 0, 2.5e6)  # Airspy standard
            sdr.setFrequency(SOAPY_SDR_RX, 0, self.fc)
            # Gain stages available
...
class SpectrumViewer(QtWidgets.QWidget):
    def __init__(self, fs=SAMPLE_RATE, N=FFT_SIZE):
        ...
        # ---- SDR thread (use nominal IF center as hw tuning reference)
