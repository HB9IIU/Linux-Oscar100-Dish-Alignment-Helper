...
# - SNR view: noise floor normalized to 0 dB
# - Markers fixed at nominal downlink freqs (shown as 489.xxx MHz = 10489.xxx - 10000)
# - Offset (kHz) shifts ONLY the spectrum trace vs fixed markers
# - Offset is absolute, saved on every change, reloaded on startup
# - Buttons: Zoom ± (long-press), Offset ± (long-press), NB Full, Zoom Beacon, Color, Quit
...
OFFSET_STEP_KHZ     = 0.1      # kHz per press
ZOOM_BEACON_FACTOR  = 0.025    # fraction of NB span
REPEAT_INTERVAL_MS  = 100      # long-press repeat speed

# SNR estimation window around beacon (exclude beacon itself)
NOISE_WINDOW_KHZ = 10.0        # ±10 kHz around beacon center
EXCLUDE_KHZ      = 3.0         # exclude ±3 kHz around beacon

OFFSET_FILE = "lnb_offset.txt" # stores absolute offset in kHz

# Display shift: show 489.xxx MHz instead of 10489.xxx (subtract 10 GHz)
...
class SDRWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        rbw = self.fs / self.N  # resolution bandwidth
...
class SpectrumViewer(QtWidgets.QWidget):
    def __init__(self, fs=SAMPLE_RATE, N=FFT_SIZE):
        ...
        self.plot.setLabel("bottom", "Frequency (MHz)")  # shows 489.xxx
        self.plot.setLabel("left", "SNR (dB)")
        layout.addWidget(self.plot)

        # Custom tick spacing for X axis (frequency)
        ...
        # ---- Build initial display axis (in 489.xxx MHz)
        self.recompute_display_axis()

        # ---- Spectrum curve (default color)
        self.colors = [
        ...
        # ---- Markers at nominal DL (displayed as 489.xxx)
        ...
        # ---- Buttons with long-press on Zoom ± and Offset ±
