...
class SDRWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        if airspy_devs:
            ...
            args = {"driver": "hackrf"}
            if "serial" in hackrf_devs[0]:
                args["serial"] = hackrf_devs[0]["serial"]
        ...
        try:
            if sdr_type == "airspy":
                ...
                # Keep same style as your HackRF snippet
                sdr.setGain(SOAPY_SDR_RX, 0, "LNA", 32)
                sdr.setGain(SOAPY_SDR_RX, 0, "VGA", 10)
        except Exception:
            sdr.setGain(SOAPY_SDR_RX, 0, 40)

        # Stream loop (same for all)
...
class SpectrumViewer(QtWidgets.QWidget):
    def __init__(self, fc=741.5e6, fs=3e6, N=8192):
        ...
        # Hide the little "A" auto-scale button
        ...
        # Force axis text and ticks to pure white
    ...
    def update_curve(self, P_db):
        ...
        if self.noise_locked and self.noise_value is not None:
            ...
            # Plateau estimate (smoothing: rolling median + EMA)
            ...
            # (Kept) plateau lines positioning, though addItem is commented above
    ...
    def mousePressEvent(self, event):
        # Exit app on any screen touch/click
...
def mainOLD():
    ...
    # Detect device and pick correct fs/N
    devs = [dict(d) for d in SoapySDR.Device.enumerate()]
    airspy_devs = [d for d in devs if str(d.get("driver", "")).lower() == "airspy"]
    rtl_devs    = [d for d in devs if str(d.get("driver", "")).lower() == "rtlsdr"]
    hackrf_devs = [d for d in devs if str(d.get("driver", "")).lower() == "hackrf"]
    ...
    if airspy_devs:
        ...
        # Using your HackRF example baseline (fs=3e6, N=8192*5)
